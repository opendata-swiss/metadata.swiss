...
public class CatalogTest extends BaseSystemTest {
    ...
    @Test
    @DependsOn(Goal.HUB_READY)
    @Provides(Goal.ODSN_CATALOG_SHOWCASES_CREATED)
    public void createCatalogShowcases(TestContext context) throws IOException {
        final long timestamp = System.currentTimeMillis();
        final String catalogId = "showcases-catalog-" + timestamp;
        final String catalogTitle = "Showcases Catalog " + timestamp;

        String catalogTurtle = ResourceUtils.loadTurtle("/catalog-odsn-showcases.ttl", catalogTitle);
        ...
        context.store(Goal.ODSN_CATALOG_SHOWCASES_CREATED, "id", catalogId);
        context.store(Goal.ODSN_CATALOG_SHOWCASES_CREATED, "iri", catalogIRI);
    }
}
